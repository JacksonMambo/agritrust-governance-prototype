...
/* Ledger core is shared with the Node test suite - see assets/js/ledger.js */
const LEDGER  = globalThis.AgriTrustLedger;
const canon   = LEDGER.canon;
const sha256Hex = LEDGER.sha256Hex;
const LEDGER_SCHEMA = LEDGER.LEDGER_SCHEMA;
...
function mkDelivery(d){
  ...
  return Object.assign({
    ...
    dispute_code:null, dispute_reason:null, dispute_notes:null,
    dispute_resolution:null, dispute_evidence_hash:null, dispute_evidence_url:null,
...
function appendEvent(event_type, actor_user_id, delivery_ref, data){
  return LEDGER.appendEvent(state.ledger, event_type, actor_user_id, delivery_ref, data);
}
...
function renderDetail(){
  ...
  document.getElementById("d_dispute").textContent =
    (d.status==="DISPUTED"||d.dispute_code)
...
/* ========= Verify chain ========= */
function verifyChain(delivery_ref, simulateTamper){
  return LEDGER.verifyChain(state.ledger, delivery_ref, !!simulateTamper);
}
...
/* ========= Audit Pack (FIXED: no <script> tags inside HTML string) ========= */
function downloadAuditPackHTML(){
  ...
  (async ()=>{
    /* An audit document must never default to PASS. If verification cannot be
       completed the pack says so rather than asserting integrity it did not check. */
    ...
    const badgeClass = (status === "PASS") ? "good" : (status === "FAIL" ? "bad" : "warn");
    const badgeText  = (status === "PASS") ? "PASS" : (status === "FAIL" ? "FAIL" : "UNVERIFIED");
...
<body>
  <div class="page">
    ...
    <div class="muted">This export includes the payload used for regulator verification.</div>
